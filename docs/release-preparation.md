...
Candidate: **sashimi-si 0.2.0rc1**; core dependency:
`sashimi-itamae>=0.2.0rc1,<0.3`. Python 3.11–3.13 is the required test matrix.

```bash
python -m pip install --find-links /path/to/reviewed-wheelhouse "sashimi-si==0.2.0rc1"
```
...
`SASHIMI_SI_SOURCE_REVISION`; the embedded source revision must survive.
